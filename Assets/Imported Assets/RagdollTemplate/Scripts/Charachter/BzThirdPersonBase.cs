...
namespace BzKovSoft.RagdollTemplate.Scripts.Charachter
{
	...
	public abstract class BzThirdPersonBase : MonoBehaviour, IBzRagdollCharacter, IBzThirdPerson
	{
		...
		// constants:
		const float JumpPower = 8.5f;		// determines the jump force applied when jumping (and therefore the jump height)
		const float AirSpeed = 6.5f;		// determines the max speed of the character while airborne
		const float AirControl = 1f;	// determines the response speed of controlling the character while airborne
        ...
        public IEnumerator CheckIfFalling()
        {
            checking = true;
		...
		void FixedUpdate()
		{
            ...
            if (PlayerTouchGound() == false && checking == false && !_jumpPressed)
            {
                StartCoroutine(CheckIfFalling());
            }
            else
            if (checking == false)
                _onGround = PlayerTouchGound() && !_jumpPressed;
